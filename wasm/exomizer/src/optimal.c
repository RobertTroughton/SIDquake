...
/*
 * ALTERED FROM UPSTREAM EXOMIZER by the SIDquake project - see
 * wasm/exomizer/README.md. The statistics tables below were declared with a
 * hard-coded 1000000 entries, which is 68 MB of static data. A WebAssembly
 * module must reserve its static data up front, so that alone forced a ~96 MB
 * initial heap on every export.
 *
 * The bound is now a macro that DEFAULTS TO THE UPSTREAM VALUE, so an ordinary
 * native build of these sources is unchanged. scripts/build-exomizer-wasm.sh
 * overrides it to 65536: the tables are indexed by match offset and match
 * length, both of which exomizer caps at 65535 (-m / -M, whose defaults the
 * wrapper never overrides), so entries above 65535 are only ever read back as
 * the zeros they were memset to.
 *
 * Two places referenced the old size numerically and now use the macro: the
 * bounds checks in optimize1() and the suffix-sum loop in optimal_optimize()
 * that used to count down from 999998. At the default the generated code is
 * identical to upstream; at 65536 the WASM build produces byte-for-byte the
 * same output as an unmodified native exomizer (verified against the images in
 * the exporter's test corpus).
 */
#ifndef EXO_OPTIMAL_STATS_SIZE
#define EXO_OPTIMAL_STATS_SIZE 1000000
#endif
                                       ...
                                       int depth, int init)
{
    ...
    do
    {
        ...
        for (i = 0; i < 16; ++i)
        {
            ...
            if (start < EXO_OPTIMAL_STATS_SIZE)
            {
                start_count = arg->stats[start];
                if (end < EXO_OPTIMAL_STATS_SIZE)
                {
                      ...
                      void *matchp_enum)        /* IN */
{
    ...
    struct interval_node **offset;
    static int offset_arr[8][EXO_OPTIMAL_STATS_SIZE];
    static int offset_parr[8][EXO_OPTIMAL_STATS_SIZE];
    static int len_arr[EXO_OPTIMAL_STATS_SIZE];
    int treshold;
    ...
    for (i = EXO_OPTIMAL_STATS_SIZE - 2; i >= 0; --i)   /* ALTERED: was 999998 */
    {
        for (j = 0; j < 8; ++j)
        {
