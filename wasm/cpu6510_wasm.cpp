...
extern "C" {
    ...
    EMSCRIPTEN_KEEPALIVE
        ...
        case 0x60: // RTS
        {
            // Pull PCL then PCH in separate statements: the order of two pop()
            // calls in one expression is unspecified in C++.
            uint8_t lo = pop();
            uint8_t hi = pop();
        ...
        case 0x40: // RTI
        {
            // Pull status, then PCL and PCH in a defined order (two pop() calls
            // in one expression are unsequenced in C++).
            cpu.status = pop() | FLAG_UNUSED;
            uint8_t lo = pop();
            uint8_t hi = pop();
            pc = lo | (hi << 8);
        ...
        // Illegal NOPs come in three distinct forms with different sizes,
        // cycle counts and memory-access behaviour; each is decoded separately
        // so maxCycles timing and MEM_READ tracking stay accurate.
        // Cycles/sizes per opcodeTable[].

        // NOP zp (2 bytes, 3 cycles, reads zp)
        case 0x04: case 0x44: case 0x64:
        { uint8_t zp = cpu.memory[pc++]; rd(zp); add(3); } break;

        // NOP zp,X (2 bytes, 4 cycles, reads (zp+X)&$FF)
        case 0x14: case 0x34: case 0x54: case 0x74: case 0xD4: case 0xF4:
        { auto e = ea_zpx(pc); rd(e.addr); add(4); } break;

        // NOP #imm (2 bytes, 2 cycles, no memory access)
