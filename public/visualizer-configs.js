...
class VisualizerConfig {
    ...
    async loadConfig(visualizerId) {
        ...
        try {
            // no-cache: always revalidate with the server. The CDN serves these
            // with max-age=300, so a plain fetch could show a config (and its
            // option list) from before the latest deploy for up to 5 minutes.
            const response = await fetch(visualizer.config, { cache: 'no-cache' });
            if (!response.ok) {
