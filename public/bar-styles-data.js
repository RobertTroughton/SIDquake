// bar-styles-data.js - Bar Style Character Data for SIDquake Web
...
// - Only main bar chars are stored (10 chars × 8 bytes = 80 bytes per style)
// - Mirror chars are computed by reversing byte order
// - Reflection frames are computed by masking mirrored chars down to a sparse
//   (~25%) shimmer: only every other scanline is lit, and the two flicker
//   frames light opposite rows (and opposite column phase) so the water
//   sparkles without ever darkening the bar's colour
...
// Sparse (~25%) shimmer: each frame lights only every other scanline, in a
// dotted pattern. The two flicker frames light opposite rows and opposite
// column phase, so over the two-frame cycle the whole reflection sparkles
// while never showing more than a quarter of the pixels at once.
//   Frame 1:  x-x-x-x- / -------- / x-x-x-x- / ...   (even rows, phase 0xAA)
//   Frame 2:  -------- / -x-x-x-x / -------- / ...   (odd rows,  phase 0x55)
function generateWaterData(styleIndex) {
    ...
    for (let c = 0; c < CHARS_PER_STYLE; c++) {
        ...
        for (let b = 0; b < BYTES_PER_CHAR; b++) {
            // Frame 1: even rows lit (0xAA dots), odd rows blank
            result[baseOffset1 + b] = (b % 2 === 0) ? (mirror[b] & 0xAA) : 0x00;
            // Frame 2: odd rows lit (0x55 dots), even rows blank
            result[baseOffset2 + b] = (b % 2 === 0) ? 0x00 : (mirror[b] & 0x55);
        }
    }
