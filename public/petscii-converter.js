...
class PETSCIIConverter {
    ...
    matchCell(imageData, cellX, cellY, charset, bgColorIndex) {
        ...
        // Precompute the squared distance from every pixel to every palette
        // colour ONCE per cell: dist[pi*16 + c]. This turns the hot char-matching
        // loop into 64*16 distance computations up front plus a plain table
        // lookup, rather than recomputing distances per candidate character.
        const dist = new Int32Array(64 * 16);
        for (let pi = 0; pi < 64; pi++) {
            const r = cellR[pi], g = cellG[pi], b = cellB[pi];
            const base = pi * 16;
            for (let c = 0; c < 16; c++) {
                const pc = this.C64_PALETTE[c];
                const dr = r - pc[0], dg = g - pc[1], db = b - pc[2];
                dist[base + c] = dr * dr + dg * dg + db * db;
            }
        }

        // Error if the whole cell rendered as background (the blank-char case).
        let allBgError = 0;
        for (let pi = 0; pi < 64; pi++) allBgError += dist[pi * 16 + bgColorIndex];

        // Reused per char: the flat indices (pixel*16) of the char's SET pixels.
        const setPixels = this._setPixelsBuf || (this._setPixelsBuf = new Int32Array(64));
        ...
        for (let ch = 0; ch < 256; ch++) {
            ...
            // One pass: collect the set-pixel offsets and their background-error
            // contribution. setBits is just the count. Only the set pixels vary
            // with fg; every unset pixel always contributes its bg distance.
            let setBits = 0;
            let setBgSum = 0;
            for (let row = 0; row < 8; row++) {
                let bits = charset[charOffset + row];
                if (!bits) continue;
                const rowBase = row * 8 * 16;
                for (let col = 0; col < 8; col++) {
                    if (bits & 0x80) {
                        const off = rowBase + col * 16;
                        setPixels[setBits++] = off;
                        setBgSum += dist[off + bgColorIndex];
                    }
                    bits = (bits << 1) & 0xFF;
                }
            }
            ...
            if (setBits === 0) {
                ...
                if (allBgError < bestError) {
                    bestError = allBgError;
                    bestChar = ch;
                    bestColor = 0;
                }
                continue;
            ...
            // error(fg) = (bg error of the unset pixels) + sum over set pixels of
            // their fg distance. The first term is constant per char; the running
            // sum is monotonic, so the >= bestError early-exit stays valid.
            const unsetBgError = allBgError - setBgSum;
            ...
            for (let fg = 0; fg < 16; fg++) {
                ...
                let error = unsetBgError;
                for (let k = 0; k < setBits; k++) {
                    error += dist[setPixels[k] + fg];
