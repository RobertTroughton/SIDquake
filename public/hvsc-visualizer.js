...
window.hvscVisualizer = (function () {
    ...
    const FLOOR = 0.07;     // 0..1 noise-floor gate; higher = more gaps between bars
    const SLOPE = 0.6;      // treble tilt compensation; higher = more high-freq boost
    // How much of a bar's level comes from its band's average bin vs its
    // loudest bin. Wide (high-frequency) bands cover hundreds of FFT bins, so
    // pure max lets a single harmonic peg the whole bar and the display
    // saturates into a "wall"; the average only rises when the band as a
    // whole carries energy. Keep some max for punch on narrow bands.
    const AVG_WEIGHT = 0.65;
    // Contrast curve applied to the final level: >1 stretches the top of the
    // range so full-height bars mean genuinely dominant bands, and mid-level
    // content reads as mid-height instead of near-full.
    const GAMMA = 1.5;
    ...
    function step() {
        ...
        // Demo mode: while nothing plays, run a gentle wave across the bars so
        // the visualizer reads as alive rather than a dead flat baseline. Two
        // travelling sines at different rates keep it from looking mechanical.
        const demoT = active ? 0 : performance.now() / 1000;
        ...
        for (let b = 0; b < BARS; b++) {
            ...
            if (!active) {
                const x = b / BARS;
                const w = 0.5 + 0.5 * Math.sin(x * Math.PI * 4 - demoT * 2.4);
                const w2 = 0.5 + 0.5 * Math.sin(x * Math.PI * 7 + demoT * 1.5);
                t = 0.03 + 0.10 * w * w + 0.04 * w2;
            }
            if (active) {
                // Bar target = blend of the band's loudest bin (punch) and its
                // average (honesty across wide bands) — see AVG_WEIGHT.
                let m = 0, sum = 0;
                for (let i = bandLo[b]; i < bandHi[b]; i++) {
                    const v = freq[i];
                    if (v > m) m = v;
                    sum += v;
                }
                const avg = sum / (bandHi[b] - bandLo[b]);
                t = ((1 - AVG_WEIGHT) * m + AVG_WEIGHT * avg) / 255;
                ...
                t *= 1 + SLOPE * (b / (BARS - 1));
                if (t > 1) t = 1;
                // Contrast: full height only for genuinely dominant bands.
                t = Math.pow(t, GAMMA);
            }
