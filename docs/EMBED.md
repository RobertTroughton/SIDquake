...
| `origin` | an origin, e.g. `https://your-site.example` | Where results are posted. Falls back to the referrer's origin. **Set it:** if no origin can be resolved, `file`/`link` selections are refused (the widget posts an `hvsc:error` instead of handing tune data to an unknown parent) and `play` announces metadata only. |
